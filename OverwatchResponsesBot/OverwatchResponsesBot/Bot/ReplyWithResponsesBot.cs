...
using OverwatchResponsesBot.Services;
using OverwatchResponsesBot.Utility;
...
namespace OverwatchResponsesBot.Bot
{
    public class ReplyWithResponsesBot
    {
        ...
        void ConfigureDatabase(string filePath, bool recreateDatabase)
        {
            if (filePath != "")
            {
                ...
                else if(File.Exists(filePath))
                {
                    ...
                    {
                        Debug.LogException("Unable to load database. Creating a new one", e);
        ...
        void PopulateDatabase(string filePath)
        {
            ...
            OverwatchGamepediaParser responsesParser = new OverwatchGamepediaParser(Constants.CATEGORY);
        ...
        public void Update()
        {
            Debug.LogImportant("Started ReplyToResponses bot");
            ...
            while (true)
            {
                ...
                //If total time to reply is less than X seconds, wait a little bit
                if ((DateTime.Now - scanStartTime).TotalSeconds < Constants.SCAN_DURATION_WAIT_SECONDS)
                {
                    int seconds = 10;
                    Debug.Log($"Scan completed quickly. Waiting {seconds} seconds...");
